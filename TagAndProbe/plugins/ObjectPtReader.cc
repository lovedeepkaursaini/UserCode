...
template<typename T>
class ObjectPtReader : public edm::EDProducer {
    public:
        explicit ObjectPtReader(const edm::ParameterSet & iConfig);
        virtual ~ObjectPtReader() ;
...
template<typename T>
ObjectPtReader<T>::ObjectPtReader(const edm::ParameterSet & iConfig) :
...
template<typename T>
ObjectPtReader<T>::~ObjectPtReader()
{
}

template<typename T>
void 
ObjectPtReader<T>::produce(edm::Event & iEvent, const edm::EventSetup & iSetup) {
    using namespace edm;
    ...
    for (object = objects->begin(); object != endobjects; ++object) {
      ...
      if(count<object->pt())count = object->pt();
    }
...
typedef ObjectPtReader<reco::Candidate> CandPtReader;
typedef ObjectPtReader<reco::Track>     TrackPtReader;

#include "FWCore/Framework/interface/MakerMacros.h"
DEFINE_FWK_MODULE(CandPtReader);
DEFINE_FWK_MODULE(TrackPtReader);
