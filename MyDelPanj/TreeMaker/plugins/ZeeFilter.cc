...
#include "CommonTools/UtilAlgos/interface/TFileService.h"
   ...
   /*
     if((e1.pt()<21)||(e2.pt()<21)){
     std::cout<<"Caught\n";
     std::cout<< "Pt1: "<<e1.pt()<<std::endl;
     std::cout<< "Pt2: "<<e2.pt()<<std::endl;
     std::cout<<"Print CutMap for e1: \n";
     for(std::map<std::string, bool>::iterator iter = leadPass.begin(); iter!=leadPass.end(); iter++)
	{
	std::cout<<"\t"<<iter->first<<"\t"<<iter->second<<std::endl;	
	}
std::cout<<"Print CutMap for e2: \n";
     for(std::map<std::string, bool>::iterator iter = subLeadPass.begin(); iter!=subLeadPass.end(); iter++)
        {
        std::cout<<"\t"<<iter->first<<"\t"<<iter->second<<std::endl;    
        }
     }
*/
     ...
     if((e1.threeCharge()*e2.threeCharge())>0)return false;
     count++;
