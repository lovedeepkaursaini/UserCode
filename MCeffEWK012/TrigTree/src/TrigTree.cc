...
// $Id: TrigTree.cc,v 1.3 2010/06/25 18:35:06 lovedeep Exp $
//
//
...
TrigTree::TrigTree(const edm::ParameterSet& iConfig):
  ...
  fillGenInfo_(iConfig.getUntrackedParameter<bool> ("fillGenInfo",1)){
  ...
  if(fillGenInfo_)genInfoTree_=new genInfoTree("TrigAnls",tree,iConfig);
...
TrigTree::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  if(fillGenInfo_)genInfoTree_->Clear();
