...
class eleScEtGreater {
  public:
  template <typename T> bool operator () (const T& i, const T& j) {
    reco::SuperClusterRef superclusteri = i.superCluster();
    reco::SuperClusterRef superclusterj = j.superCluster();
    
    double et1=superclusteri->energy()/std::cosh(superclusteri->eta());
    double et2=superclusterj->energy()/std::cosh(superclusterj->eta());
    return (et1>et2);
  }
};
